...
def load_referential_data(config, referit_csv, scans_split):
    """
    :param config:
    :param referit_csv:
    :param scans_split:
    ...
    if config.mentions_target_class_only:
    ...
    n_original = len(referit_data)
    referit_data = referit_data[referit_data.tokens.apply(lambda x: len(x) <= config.max_seq_len)]
    referit_data.reset_index(drop=True, inplace=True)
    print('Dropping utterances with more than {} tokens, {}->{}'.format(config.max_seq_len, n_original, len(referit_data)))

    # do this last, so that all the previous actions remain unchanged
    if config.augment_with_sr3d is not None:
        print('Adding Sr3D as augmentation.')
        sr3d = pd.read_csv(config.augment_with_sr3d)
...
def compute_auxiliary_data(referit_data, all_scans, config):
    ...
    if config.vocab_file:
        ...
        vocab = build_vocab([x for x in train_tokens], config.min_word_freq)
        print(('Length of vocabulary, with min_word_freq={} is {}'.format(config.min_word_freq, len(vocab))))
